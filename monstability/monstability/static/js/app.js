...
path = biHiSankey.link().curvature(0.15);
...
function getData() {
  ...
  client.get(URL_Host+'/nodes/?format=json', function(response) {
    var jsnArray = JSON.parse(response);
    exampleNodes = new Array();
    for (var newExampleNode of jsnArray) {
      var xPOS = 0;
    ...
    client1.get(URL_Host+'/edges/?format=json', function(response) {
      var jsnArray = JSON.parse(response);
      exampleLinks = new Array();
      for (var newExampleNode of jsnArray) {
        exampleLinks.push({"source":newExampleNode.source, "target":newExampleNode.target, "value":newExampleNode.weight});
       // exampleLinks.push({"source":newExampleNode.source, "target":newExampleNode.target, "value":0.1});
      }
      build();
    });

  //  return Types;// do something with response
  });
...
function build() {
  // if (exampleLinks == null || exampleNodes == null){
  //   build();
  // } else {
    biHiSankey
        .nodes(exampleNodes)
  ...
  if (!AUTO_POSITION) {
    exampleLinks.forEach((r, i) => {
     // if(r.xPOS!=0){
     //  r.sourceY = r.sourceY+r.source.height/3;
     //  r.targetY = r.targetY+r.target.height/3;
      r.thickness = r.thickness/4;
    //  }
      //  r.y = r.yPos;
    });
